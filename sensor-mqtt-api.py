...
while True:
    ...
    api_url = "http://127.0.0.1:5000/api/add_data"
    payload = {
        "temperature": sensor_data.get('temp', 0.0),
        "moisture": sensor_data.get('humi', 0.0),
        "pH": sensor_data.get('phph', 0.0),
        "conductivity": sensor_data.get('cond', 0.0),
        "nitrogen": sensor_data.get('nitro', 0.0),
        "phosphorus": sensor_data.get('phos', 0.0),
        "potassium": sensor_data.get('pota', 0.0),
        "tanaman": "cabai",  # Ganti sesuai kebutuhan
        "tanaman_no": 1       # Ganti sesuai kebutuhan
    }

    try:
        response = requests.post(api_url, json=payload)  # Kirim data ke API
    ...
    print("############")
    time.sleep(30)  # Tunggu 30 detik sebelum pembacaan berikutnya
